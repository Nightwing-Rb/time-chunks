...
def extract_elements(pdf_path: str, output_dir: str) -> List[FlattenedElement]:
    ...
    if not os.path.exists(json_path):
        raise RuntimeError(f"Extraction failed. Expcted JSON at {json_path}")
...
def _merge_chapter_numbers(elements: List[FlattenedElement]) -> List[FlattenedElement]:
    ...
    for i, element in enumerate(elements):
        ...
        is_number_heading = (
            ...
            and re.match(r"^\d+$", element.content.strip())
        )
        ...
        if (i + 1 < len(elements)
            ...
            if element.heading_level and nxt.heading_level and nxt.heading_level <= element.heading_level:
                
                new_content = f"Chapter {num}: {nxt.content}"
                merged.append(FlattenedElement(
                    type="heading",
                    content=new_content,
...
def _flatten_node(node: Dict[str, Any], result: List[FlattenedElement]):
    ...
    if node_type == "image":
        ...
        if image_source:
             result.append(FlattenedElement(
                 type="image",
    ...
    # ignore images and headers/footers for now
