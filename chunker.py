...
def generate_chunks(
    elements: List[FlattenedElement], words_per_minute: int, duration_minutes: float
) -> List[Chunk]:
    """
    ...
    heading_min = int(
        target_words * 0.5
    )  # Break if we encounter a heading and already have half a chunk
    ...
    if len(chunks) > 1:
        # Handle first chunk being a runt by merging into second
        if chunks[0].total_words < min_chunk_words:
            chunks[1].elements = chunks[0].elements + chunks[1].elements
            chunks[1].total_words += chunks[0].total_words
            chunks[1].estimated_minutes = round(
                chunks[1].total_words / float(words_per_minute), 1
            )
            chunks = chunks[1:]
        ...
        for ch in chunks[1:]:
            if ch.total_words < min_chunk_words and merged:
                ...
                prev.estimated_minutes = round(
                    prev.total_words / float(words_per_minute), 1
                )
            else:
                merged.append(ch)
